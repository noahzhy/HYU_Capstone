...
class PeleeNet:
    ...
    def peleenet(self, input_x, k=32, num_init_channel=64, block_config=[3, 4, 8, 6], bottleneck_width=[2, 2, 4, 4], is_training=True, reuse=False):
        with tf.variable_scope(self.model_name) as scope:
            ...
            logits = self.layer._classification_layer(
                from_layer, self.num_class, is_training=is_training, reuse=reuse)
            return logits

    def build_model(self):
        self.input_train = tf.placeholder(tf.float32, [
                                          self.batchsize, self.input_height, self.input_width, self.input_channel], name='input_train')
        self.input_test = tf.placeholder(tf.float32, [
                                         self.batchsize, self.input_height, self.input_width, self.input_channel], name='input_test')
        self.one_hot_labels = tf.placeholder(
            tf.float32, [self.batchsize, self.num_class], name='one_hot_labels')

        # logits data and one_hot_labels
        self.logits_train = self.peleenet(
            self.input_train, is_training=True, reuse=False)
        self.logits_test = self.peleenet(
            self.input_test, is_training=False, reuse=True)
        ...
        self.weights_reg = tf.reduce_sum(
            tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES))
        self.loss = tf.reduce_mean(softmax_cross_entropy_with_logits(
            self.logits_train, self.one_hot_labels)) + self.config.weight_decay*self.weights_reg
        ...
        self.accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(
            self.predicetion, 1), tf.argmax(self.one_hot_labels, 1)), tf.float32))
    ...
    def test_model(self):
        ...
        gen_data = gen_batch_data(
            datasource, self.batchsize, is_training=False)
        ites_per_epoch = int(len(datasource.images)/self.batchsize)
...
if __name__ == '__main__':
    ...
    peleenet = PeleeNet(config=config)
    for i in range(10):
